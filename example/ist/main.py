...
def prepare_ws_data(media_name, payload_path):
    ...
    if media_type in ["audio"]:
        ...
        media_data_list = ne_utils.build_stream_data(payload, send_ws=True)
...
def prepare_req_data(request_data):
    ...
    """
    media_path2name = {}
    for media_type in client.media_type_list:
...
def on_open(ws):
    def run():
        ...
        # 获取待发送的数据
        media_path2data = prepare_req_data(request_data)
        # 发送数据
        send_ws_stream(ws, request_data, media_path2data, multi_mode, time_interval)
...
if __name__ == '__main__':
    cli = client.Client(APPId, APIKey, APISecret, "ws://cn-huadong-1.xf-yun.com", "/v1/private/s501d1f86")
    cli.stream(on_open=on_open, on_message=on_message, on_error=on_error)